import { pickupHotspots } from "@/data/taxiData";
      ...
      >
        ...
        <div className="p-3">
          ...
          >
            ...
            {/* Hotspot connections (subtle lines between related spots) */}
            {pickupHotspots.slice(0, 3).map((spot, i) => {
              const next = pickupHotspots[(i + 1) % 3];
              const p1 = project(spot.lat, spot.lng);
              const p2 = project(next.lat, next.lng);
              return (
                <line key={`conn-${i}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke="#2d2d2d" strokeOpacity="0.1" strokeWidth="1" strokeDasharray="4 6" />
              );
            })}

            {/* Hotspots with pulsing effect */}
            {pickupHotspots.map((spot) => {
              const { x, y } = project(spot.lat, spot.lng);
              const r = (spot.intensity / 100) * 28 + 8;
              return (
                <g key={spot.name}>
                  {/* Outer glow */}
                  <circle cx={x} cy={y} r={r + 4} fill="#ff4d4d" fillOpacity={0.08} />
                  {/* Main circle */}
                  <circle cx={x} cy={y} r={r} fill="#ff4d4d" fillOpacity={0.2} stroke="#ff4d4d" strokeWidth="2" strokeDasharray={spot.intensity > 70 ? "0" : "4 3"} />
                  {/* Center dot */}
