...
if [ ! -f "$PROJECT_ROOT/Data/cleaned/cleaned_taxi_data.csv" ]; then
  echo "WARNING: Data/cleaned/cleaned_taxi_data.csv not found."
  echo "Skipping Python export step. Using existing Data/frontend/ CSVs."
  echo "To regenerate: run Notebooks/01_cleaning.ipynb then python src/export_frontend.py"
  # Skip to copy step
  echo "Step 2: Copying existing Data/frontend/ CSVs to frontend public/data/..."
  mkdir -p "$FRONTEND_PUBLIC"
  cp Data/frontend/*.csv "$FRONTEND_PUBLIC/" 2>/dev/null || true
else
